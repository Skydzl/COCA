...
def train(feature1, feature_dec1, center, length, epoch, config, device):
    ...
    sigma_loss1 = torch.max(torch.zeros_like(sigma_aug1), (1 - sigma_aug1)) # v(Q)
    sigma_loss2 = torch.max(torch.zeros_like(sigma_aug2), (1 - sigma_aug2)) # v(Q')
    loss_sigam = torch.mean((sigma_loss1 + sigma_loss2) / 2) # variance term
